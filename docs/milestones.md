...
| Local model feasibility | Complete | Hardware, runtime options, candidate model tiers, and provider requirements are documented. | [local_model_feasibility.md](local_model_feasibility.md) | A working runtime still needs prompt and schema tuning before it becomes a useful baseline. | Use the live Ollama path to mature M-A2. |
...
| Visibility Phase 3: local dashboard | Complete | A generated static Evidence Notebook dashboard renders current claims, milestones, recent sessions, active threads, artifacts, and decisions. | [site/index.html](../site/index.html), [src/epilepsy_agents/visibility/](../src/epilepsy_agents/visibility/), [tests/test_visibility.py](../tests/test_visibility.py) | Dashboard regenerates manually. | Regenerate with `epilepsy-agents notebook` after substantial state updates. |
...
| --- | --- | --- | --- | --- | --- |
| M-A1 Local runtime live | Complete | A local Ollama server is running and responds through `src/epilepsy_agents/providers.py`; `provider-smoke` succeeds end to end against `qwen3.5:4b`. | [local_model_feasibility.md](local_model_feasibility.md), [src/epilepsy_agents/providers.py](../src/epilepsy_agents/providers.py), [src/epilepsy_agents/cli.py](../src/epilepsy_agents/cli.py), [20260424T160537Z_local_provider_smoke_path.md](run_logs/20260424T160537Z_local_provider_smoke_path.md) | The successful runtime is currently Ollama-specific rather than a broader runtime bake-off, and latency may still constrain larger-slice experiments. | Use the live Ollama path as the default Phase A runtime and focus on h003 output quality. |
| M-A2 h003 single-prompt LLM smoke | In progress | `h003_single_prompt_llm` now exists and has produced a first local run record with JSON-schema validation, retries, latency, and token metadata. | [scripts/run_harness_experiment.py](../scripts/run_harness_experiment.py), [src/epilepsy_agents/llm_pipeline.py](../src/epilepsy_agents/llm_pipeline.py), [project_state/harnesses/README.md](../project_state/harnesses/README.md), [20260424T171400Z_h003_single_prompt_llm_n5.json](../project_state/runs/20260424T171400Z_h003_single_prompt_llm_n5.json) | The first n=5 smoke has invalid-output rate 0.80 and mean latency about 52.9 seconds, so the prompt/schema path is not yet strong enough for a 25-50 row comparison. | Inspect the four `unknown` outputs, tune prompt/schema behavior, and rerun h003 on a 25-row slice. |
