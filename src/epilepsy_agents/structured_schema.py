...
def system_prompt() -> str:
    return (
        ...
        "Worked label examples (synthetic phrasings, not from any real letter):\n"
        "- 'two events each week' -> '2 per week'\n"
        "- 'six episodes over the past seven months' -> '6 per 7 month'\n"
        "- 'roughly one seizure every six weeks' -> '1 per 6 week'\n"
        "- 'two cluster days per month, around six seizures per cluster' -> "
        "'2 cluster per month, 6 per cluster'\n"
        "- 'seizure free for several months since the last review' -> "
        "'seizure free for multiple month'\n"
        "- 'multiple seizures per week, exact count not given' -> 'multiple per week'"
    )
