...
def monthly_rate_match(gold: float | None, predicted: float | None, tolerance: float = 0.15) -> bool:
    ...
    if gold == 0.0 or predicted == 0.0:
        return gold == predicted
    return abs(gold - predicted) <= max(tolerance, abs(gold) * tolerance)


def evaluate_prediction(source_row_index: int, gold_label: str, prediction: Prediction) -> EvaluationRow:
